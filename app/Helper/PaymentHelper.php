...
class PaymentHelper
{
    ...
    public static function get_commission_amount()
    {
        $commission_settings=CommissionSettings::get()->first();
        $commission_amount=$commission_settings->amount;
        ...
        foreach($accounts as $account)
        {
            ...
            if($balance >= $settings_amount)
            {
                ...
                { 
                    ...
                    $subscriptions->commission_amount=static::get_commission_amount();
                    $subscriptions->save(); 
                ...
                else
                {
                    ...
                    $payments = SubscriptionPayment::find($id);
 	                $payments->subscription_fee= session('settings_amount');
        	        $payments->gst= session('gst');
                    $payments->paid_date=date('Y-m-d');
                    $payments->fk_transaction_id=$trans_id;
                    $payments->status="Paid";
                    $payments->commission_amount=static::get_commission_amount();
                    $payments->save();
                }    
            }
            else
    ...
    //Total earnings of all accounts
    public static function total_commission_earned($user_id)
    {
        $total_amount=0;
    ...
    //Get total commission amount earned by primary user

    public static function total_commissions_earned($user_id)
    {
        $total_amount=0;
        $accounts=Accounts::where('fk_user_id',$user_id)
        ->where('status','Activated')->get();
        ...
        {
            $account_id=$account->acct_id;
            $incomes=SubscriptionPayment::where('fk_acct_id',$account_id)
            ->where('status','Paid')->get();
            foreach($incomes as $income)
            {
                $amount_paid=$income->commission_amount;
                $total_amount += $amount_paid;
            }
            return $total_amount;
        }
    ...
    //Individual earnings of each accounts

    public static function total_earnings($id)
    {
    ...
    //Total income earned by each account

    public static function total_income_earnings($id)
    {
        $currentDate=PaymentHelper::get_current_date();
        $month=date('m',strtotime($currentDate));  
        $year=date('Y',strtotime($currentDate));  
